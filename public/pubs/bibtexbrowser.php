  ...
  */
  ...
  function addHomepageLink($author) {
    ...
    if (isset($_GET[Q_DB]->stringdb[$homepage]))
      $author='<a class="bibauthorlink" href="'.$_GET[Q_DB]->stringdb[$homepage].'">'.$author.'</a>';
    return $author;
  }
  ...
  */
  ...
  function bib2links() {
    ...
    if (BIBTEXBROWSER_BIBTEX_LINKS) {
      ...
      $str .= " <a".(BIBTEXBROWSER_BIB_IN_NEW_WINDOW?' target="_blank" ':'')." class=\"biburl\" title=\"".$this->getKey()."\" {$href}>[bib]</a>";
    }
  ...
  function toEntryUnformatted() {
    $result = "";
    $result .= '<div id="pb_wrapper" onclick="select(\'pb_wrapper\')">';
...
class NotFoundDisplay {
  ...
  function getTitle() { return @$this->title ; }
  function display() {
    echo 'Publications will be listed here.';
  }
}
...
class BibEntryDisplay {
  ...
  /** returns the title */
  function getTitle() {
      return $this->bib->getTitle();
//    return $this->bib->getTitle().' (bibtex)';
  }
  
  /** 2011/10/02: new display, inspired from Tom Zimmermann's home page */
  function displayOnSteroids() {
	  $title = "<div class=\"rheader\"><h2>" . $this->getTitle() . "</h2></div>";
      $subtitle = '<div class="bibentry-by">'.$this->bib->getFormattedAuthorsImproved().'</div>';
      ...
      $reference= '<div class="bibentry-label">Reference:</div><div id="bibentry-reference" onclick="select(\'bibentry-reference\')">'.strip_tags(bib2html($this->bib)).'</div>';

      $bibtex = '<div class="bibentry-label">Bibtex Entry:</div>'.$this->bib->toEntryUnformatted().'';
      return $title.$subtitle.$abstract.$download.$reference.$bibtex.$this->bib->toCoins();
  }
